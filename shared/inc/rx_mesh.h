...
int CalMeshDiv(Vec3 &minp, Vec3 maxp, double h, int n[3], double extend = 0.05);
...
bool SetArrayFromFBO(GLuint uVrtVBO, GLuint uNrmVBO, GLuint uTriVBO, 
					 vector<Vec3> &vrts, vector<Vec3> &nrms, vector<rxFace> &face, int nvrts, int ntris, int dim = 4);
 ...
 * @return 成功ならば0
 */
inline int CalMeshDiv(Vec3 &minp, const Vec3 maxp, const int nmax, double &h, int n[3], double extend)
{
	Vec3 l = maxp-minp;
	minp -= 0.5*extend*l;
	l *= 1.0+extend;
 ...
 * @return 成功ならば0
 */
inline int CalMeshDiv(Vec3 &minp, Vec3 maxp, double h, int n[3], double extend)
{
	Vec3 l = maxp-minp;
	minp -= 0.5*extend*l;
	l *= 1.0+extend;
...
inline bool SetArrayFromFBO(GLuint uVrtVBO, GLuint uNrmVBO, GLuint uTriVBO, 
							vector<Vec3> &vrts, vector<Vec3> &nrms, vector<rxFace> &face, int nvrts, int ntris, int dim)
{
	// 頂点アレイに格納
	glBindBuffer(GL_ARRAY_BUFFER, uVrtVBO);
