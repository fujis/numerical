 ...
 * @param[out] w,h 画像サイズ
 * @param[out] c 画像の色深度
 * @param[in] use_bmp_w ビットマップの水平方向バイト数((((w+1)*c)/4)*4)を保つ
 * @param[in] h_inv 高さ方向のピクセル反転フラグ(default=true)
 * @param[in] bgr_inv BGR->RGB変換フラグ(default=true)
 * @return 画像データ
 */
static unsigned char* ReadBitmapFile(const std::string &fn, int &w, int &h, int &c, int &wstep, 
									 bool h_inv = true, bool bgr_inv = true)
{
	// ファイルをバイナリモードで開く
	FILE *fp;
	...
	wstep = (((w+1)*c)/4)*4;	// ビットマップの水平方向(スキャンライン)のバイト数は4の倍数(余ったところは0x00で埋める)
	...
	// 読み込みのための一時保存バッファ(水平方向のバイト数が4の倍数)
	unsigned char* img_buf = new unsigned char[wstep*h];
	...
	if(!cpalette.empty()){
		...
		for(int i = 0; i < m; ++i){
			...
			for(int j = 0; j < div; ++j){
				...
				int idx = 3*(div*i+j);
				img_buf[idx+0] = (cmask[0] & c) >> cmask_shift*2;
				img_buf[idx+1] = (cmask[1] & c) >> cmask_shift;
				img_buf[idx+2] = (cmask[2] & c);
		...
		// 画像データの取得
		fread(img_buf, sizeof(unsigned char), (size_t)(long)(wstep*h), fp);
	}

	// 出力データ
	unsigned char* img = new unsigned char[wstep*h];

	// 画像データ
	int c0 = bgr_inv ? 2 : 0;
	int c2 = bgr_inv ? 0 : 2;
	// BGR -> RGB と 上下反転 しながらデータをコピー
	for(int j = 0; j < h; ++j){
		for(int i = 0; i < w; ++i){
			int idx0 = c*i+j*wstep;
			int idx1 = h_inv ? c*i+(h-j-1)*wstep : c*i+j*wstep;
			img[idx0+0] = img_buf[idx1+c0];
			img[idx0+1] = img_buf[idx1+1];
			img[idx0+2] = img_buf[idx1+c2];
		}
		for(int i = w*c; i < wstep; ++i){
			img[i+j*wstep] = 0x00;
		}
	}
 ...
 * @param[in] w,h 画像サイズ
 * @param[in] c 画像の色深度
 * @param[in] iwstep 入力画像データの水平方向のバイト数(ビットマップなら(((w+1)*c)/4)*4
 * @param[in] type ビットマップ形式(default=RX_BMP_WINDOWS_V3)
 * @param[in] h_inv 高さ方向のピクセル反転フラグ(default=true)
 * @param[in] bgr_inv RGB->BGR変換フラグ(default=true)
 */
static int WriteBitmapFile(const std::string &fn, unsigned char *img, int w, int h, int c, int type = RX_BMP_WINDOWS_V3, 
						   int iwstep = -1, bool h_inv = true, bool bgr_inv = true)
{
	// ファイルをバイナリモードで開く
	FILE *fp;
	...
	// 画像データ
	int wstep = (((w+1)*c)/4)*4;	// ビットマップの水平方向(スキャンライン)のバイト数は4の倍数(余ったところは0x00で埋める)
	unsigned char *img_buf = new unsigned char[wstep*h];
	int c0 = bgr_inv ? 2 : 0;
	int c2 = bgr_inv ? 0 : 2;
	// RGB -> BGR と 上下反転
	for(int j = 0; j < h; ++j){
		for(int i = 0; i < w; ++i){
			int idx0 = c*i+j*wstep;
			int idx1 = h_inv ? c*i+(h-j-1)*iwstep : c*i+j*iwstep;
			img_buf[idx0+0] = img[idx1+c0];
			img_buf[idx0+1] = img[idx1+1];
			img_buf[idx0+2] = img[idx1+c2];
		}
		for(int i = w*c; i < wstep; ++i){
			img_buf[i+j*wstep] = 0x00;
		}
	}

	fwrite(img_buf, sizeof(unsigned char), (size_t)(long)(wstep*h), fp);
	delete [] img_buf;
